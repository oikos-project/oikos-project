...
def test_register_route_success(client, db_session):
    response = client.post("/auth/register", json={"email": "route@example.com", "password": "StrongPassword123!@#"})
...
def test_login_route_success(client, db_session):
    ...
    response = client.post("/auth/login", json={"email": "route_login@example.com", "password": "StrongPassword123!@#"})
...
def test_login_route_invalid_credentials(client):
    response = client.post("/auth/login", json={"email": "no@example.com", "password": "bad"})
