...
def register_user_service(email, password):
    ...
    try:
        if not ALLOW_OPEN_REGISTRATION:
            print("DEBUG: register_user_service: Returning False - Registration closed.")
            return {"success": False, "message": "Registration is currently closed."}

        # Check password strength
        strength = zxcvbn(password, user_inputs=[email])
        if strength['score'] < 2:
            print(f"DEBUG: register_user_service: Returning False - Password too weak (score: {strength['score']}).")
            return {"success": False, "message": "Password is too weak.", "suggestions": strength['feedback']['suggestions']}

        existing_user = db.query(User).filter_by(email=email).first()
        if existing_user:
            print(f"DEBUG: register_user_service: Returning False - User already exists: {existing_user.email}.")
        ...
        print("DEBUG: register_user_service: Returning True - User registered successfully.")
